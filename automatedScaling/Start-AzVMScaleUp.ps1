<#  
.SYNOPSIS  
    Vertically scale up an Azure Virtual Machine
.DESCRIPTION  
    Vertically scale up an Azure Virtual Machine
...
.EXAMPLE  
    Manual exection as a PowerShell script with custom VM Size list
        Start-AzVMScaleUp.ps1 -VmName {Azure_VM-Name} -ResourceGroupName {Azure_Resource_Group_Name} -VmSizes {Comma,Seperated,Sizes}
        Start-AzVMScaleUp.ps1 -VmName "VM-Host1" -ResourceGroupName "RG-East" -VmSizes "Standard_D2s_v3,Standard_D3s_v3,Standard_D4s_v3,Standard_D6s_v3"
...
function Set-VMSize () {
    ...
    Write-Output ("Scaling up " +  $VmName + " to " + $VmSize)
...
Write-Output "Starting VM Scale Up."
...
if ($attemptScale){
    ...
    for ($x = 0; $x -lt $scaleLevel.count; $x++) {
        ...
        if ($CurrentSize -eq $scaleLevel[$x]){
            ...
            if ($x -eq $scaleLevel.count-1){
                ...
                try {
                    Write-Output ("Setting VM named " + $VmName + " to Level " + ($x+2) + " - " + $scaleLevel[$x+1] + ".")
    ...
    Write-Output "Completed VM Scale Up."
}